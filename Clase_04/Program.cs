	...
	class Program
	{
		public static void Main(string[] args)
		{
             ...
             for (int i = 0; i < 4; i++)
            {
                if (i <= 10)
                {
                    IAlumno alum = (Alumno)FabricaDeComparables.crearAleatorio(2); //creo 10 IAlumnos Alumnos aleatoriamente
					IAlumno decorar = new DecoratorLegajo(alum);
                ...
                if (i > 10)
                {
                    IAlumno alum = (AlumnoMuyEstudioso)FabricaDeComparables.crearAleatorio(3); //creo 10 IAlumnos AlumnosMuyEstudiosos aleatoriamente
                    IAlumno decorar = new DecoratorLegajo(alum);
                    decorar = new DecoratorNotaEnLetras(decorar);
                    decorar = new DecoratorNotaFinal(decorar);
                    decorar = new DecoratorOrdenSecuencial(decorar);
                    decorar = new DecoratorCuadro(decorar);	
                    maestro.goToClass(new AlumnoAdapter(alum));
                }
            }
            ...
            //Teacher maestro = new Teacher();
            //IAlumno alum = (IAlumno)FabricaDeComparables.crearAleatorio(4);
            //maestro.goToClass(new AlumnoAdapter(alum));
            //maestro.teachingAClass();
