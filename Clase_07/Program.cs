...
{		
	class Program
	{
		...
		//PRACTICA 5 EJERCICIO 2. Compruebo el correcto funcionamiento del Proxy. (Gebera siempre el mismo nombre nose porque)
           ...
           //PRACTICA 6 EJERCICIO 2. main para verifica el alumno compuesto 
            //GeneradorDeDatosAleatorios generador = new GeneradorDeDatosAleatorios();
            //Teacher teacher = new Teacher();
            //AlumnoCompuesto alumno = (AlumnoCompuesto)FabricaDeComparables.crearAleatorio(5);
            //for (int i = 0; i < 5; i++)
            //{
            	//IAlumno a = new AlumnoProxy(generador.stringAleatorio(10), generador.numeroAleatorio(50000000),generador.numeroAleatorio(9999),generador.numeroAleatorio(10),1); //creo un alumnoporxy
                //IAlumno decorar = new DecoratorLegajo(a);
                //decorar = new DecoratorNotaEnLetras(decorar);
                //decorar = new DecoratorNotaFinal(decorar);
                //decorar = new DecoratorOrdenSecuencial(decorar);
                //decorar = new DecoratorCuadro(decorar);
                //teacher.goToClass(new AlumnoAdapter(teacher));
                //alumno.agregarHijo(a);
            //}
            //Aula aula = new Aula(teacher);
            //aula.nuevoAlumno(alumno);
            //teacher.teachingAClass();
            
            //PRATICA 6 EJERCICIO 5. Verifico el primer juego creado
            //JuegoDeCartasGeneral juego = new JuegoDeCartas();
            //Persona p1= (Persona)FabricaDeComparables.crearAleatorio(6);
            //Persona p2= (Persona)FabricaDeComparables.crearAleatorio(6);
            //Persona ganador = juego.Partida(p1,p2);
            
            //PRATICA 6 EJERCICIO 6. Verifico el segundo juego.
            //JuegoDeCartasGeneral juego = new JuegoDeCartasParidad();
            //Persona p1= (Persona)FabricaDeComparables.crearAleatorio(6);
            //Persona p2= (Persona)FabricaDeComparables.crearAleatorio(6);
            //Persona ganador = juego.Partida(p1,p2);
          
            //PRACTICA 7 EJERCICIO 3. Verifico el comportamiento  (FUNCIONA Y DECORA)
            Teacher teacher = new Teacher();
            AlumnoCompuesto alumno = (AlumnoCompuesto)FabricaDeComparables.crearAleatorio(5);
            for (int i = 0; i < 5; i++)
            {
            	IAlumno a = (IAlumno)FabricaDeComparables.crearDesdeArchivo(2);
                IAlumno decorar = new DecoratorLegajo(a);
                decorar = new DecoratorNotaEnLetras(decorar);
                decorar = new DecoratorNotaFinal(decorar);
                decorar = new DecoratorOrdenSecuencial(decorar);
                decorar = new DecoratorCuadro(decorar);
                teacher.goToClass(new AlumnoAdapter(decorar)); //IMPORTANTE PARA QUE DECORE UTILIZAR GOTOCLASS 
                alumno.agregarHijo(decorar);
            }
             for (int i = 0; i < 2; i++)
            {
                IAlumno a = (IAlumno)FabricaDeComparables.crearDesdeArchivo(3);
                IAlumno decorar = new DecoratorLegajo(a);
                decorar = new DecoratorNotaEnLetras(decorar);
                decorar = new DecoratorNotaFinal(decorar);
                decorar = new DecoratorOrdenSecuencial(decorar);
                decorar = new DecoratorCuadro(decorar);
                teacher.goToClass(new AlumnoAdapter(decorar));
                alumno.agregarHijo(decorar);
             }
            Aula aula = new Aula(teacher);
            aula.nuevoAlumno(alumno);
		...
		//PRACTICA 2 EJERCICIO 2. Modifico el método llenarAlumno para que cada alumno tenga la estrategia elegida
		public static void llenarAlumnos(Coleccionable c)
		{
            Random rd = new Random();
		...
		//PRACTICA 2 EJERCICIO 14. Tres iteradores de una coleccion
		...
		//PRACTICA 6 EJERCICIO 7. Para que se permitan mas jugadores, yo utilizaria una lista de jugadores en vez de simplemente enviarle dos jugadores por parametroy ya con eso ir desarrolando para que soporte varios jugadores.
		
		//ACLARACIÓN: El patrón Chains of Responsability no lo entendi muy bien, ya que la atividad que nos dan no es muy clara acerca del problema que se pasan, pero en el ejemplo que nos dan queda claro.
